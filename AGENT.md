...
8. **Do not create these files** — they conflict with existing infrastructure and must never exist in this project:
   ...
   - `src/middleware.ts` — this project uses `src/proxy.ts` (the Next.js 16 convention). Do not create a parallel `middleware.ts`; the two would both register and conflict, and `middleware.ts` emits a deprecation warning at build time.
